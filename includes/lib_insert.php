...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, market_price FROM ' . $GLOBALS['ecs']->table('goods') .
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $goods['market_price'] = $row['market_price']."元/m<sup>2</sup>";
            $goods['url'] = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);
            $str.='<ul class="clearfix"><li class="goodsimg"><a href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" class="B_blue" /></a></li><li class="htext"><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a><br />'."价格：".'<font class="f1">'.$goods['market_price'].'</font><br /></li></ul>';
        }
